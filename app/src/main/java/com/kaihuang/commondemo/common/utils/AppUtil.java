...
public class AppUtil {
    ...
    public static void installNew(Context mContext, String filePath) {
...
//			Log.w(TAG, "正常进行安装"); "com.chnyoufu.youfu.fileprovider"
            intent.setDataAndType(Uri.fromFile(apkFile), "application/vnd.android.package-archive");
        }
        mContext.startActivity(intent);
    }

    public static void installNew2(Context var1, String var0) {
//		Log.i(TAG, "开始执行安装: " + filePath);
        File apkFile = new File(var0);
        Intent var2 = new Intent();
        var2.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        var2.setAction(Intent.ACTION_VIEW);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri uriForFile = FileProvider.getUriForFile(var1, var1.getApplicationContext().getPackageName() + ".provider", apkFile);
            var2.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            var2.setDataAndType(uriForFile, var1.getContentResolver().getType(uriForFile));
        } else {
            var2.setDataAndType(Uri.fromFile(apkFile), "application/vnd.android.package-archive");
        }
        try {
